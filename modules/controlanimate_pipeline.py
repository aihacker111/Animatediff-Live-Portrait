...
class ControlAnimatePipeline():
    def __init__(self, config):
        ...
        if not self.use_lcm:
            ...
            unet = UNet3DConditionModel.from_pretrained_2d(model_config.pretrained_lcm_model_path, subfolder="unet",
                                                           use_safetensors=True,
                                                           unet_additional_kwargs=OmegaConf.to_container(
                                                               self.inference_config.unet_additional_kwargs))

        self.multicontrolnetresiduals_pipeline = MultiControlNetResidualsPipeline(list(model_config.controlnets),
                                                                                  list(config.cond_scale),
                                                                                  use_lcm=self.use_lcm) if model_config.controlnets is not None else None
        ...
        if self.use_ipadapter:
            ...
            main_ip_pipe = IPAdapter(pipeline, image_encoder_path, ip_ckpt, 'cuda',
                                     num_tokens=4)  # IPAdapterPlus(pipeline, image_encoder_path, ip_ckpt, 'cuda', num_tokens=16)
        ...
        if not self.use_lcm:
            self.pipeline = load_weights(
                ...
                lora_model_path=model_config.get("lora_model_paths", "") if model_config.get("lora_model_paths",
                                                                                             "") is not None else "",
